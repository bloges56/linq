    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            IEnumerable<Bank> millionaires = customers.GroupBy(customer => customer.Bank, customers => customers.Balance, (key, balances) => new Bank()
            {
                name=key, amount= balances.Where(balance => balance >= 1000000).Count()
            });

            // foreach (Bank bank in millionaires)
            // {
            //     Console.WriteLine($"{bank.name} {bank.amount}");
            // }
        }
    }
}
